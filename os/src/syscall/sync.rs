...
pub fn sys_mutex_create(blocking: bool) -> isize {
    ...
    process_inner.available.mutex.insert(id as usize,1);
    for t in process_inner.allocation.as_mut_slice() {
        t.mutex.insert(id as usize,0)
    }
    for t in process_inner.need.as_mut_slice() {
        t.mutex.insert(id as usize,0)
    }
    id
}
...
pub fn sys_mutex_lock(mutex_id: usize) -> isize {
    ...
    if process_inner.deadlock_detect{
        process_inner.need[tid].mutex[mutex_id]+=1;
        if process_inner.bankers_algorithm().is_some(){
            process_inner.need[tid].mutex[mutex_id]-=1;
            process_inner.allocation[tid].mutex[mutex_id]+=1;
            process_inner.available.mutex[mutex_id]-=1;
            drop(process_inner);
            drop(process);
            mutex.lock();
            0
